    ...
    public class SoundScene : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (intervalTime < elapsedTime) // 6秒ごとに実行
            {
                ...
                byte [] notes  = new byte[] {72, 72, 72, 72, 68, 70,72,70,72};
                int [] duration  = new int[] {250, 250, 250, 500, 500, 500,250,250,1000};
                for (int i = 0; i < notes.Length; i++)
                {
	                sound.Add(new Cube.SoundOperation(duration[i], volume:15, notes[i]));
